...
const VendorSales = () => {
  ...
  const { data: orders = [] } = useQuery({
    queryKey: ["vendor-sales-orders", user?.id],
    queryFn: async () => {
  ...
  const { data: productCount = 0 } = useQuery({
    queryKey: ["vendor-product-count", user?.id],
    queryFn: async () => {
  ...
  const stats = [
    ...
    { label: "Orders Today", value: String(todayOrders.length), icon: ShoppingCart },
    ...
    <Layout>
      <section className="container py-20 md:py-28">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Sales Summary</h1>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card>
            <CardHeader><CardTitle className="text-lg">Orders (Last 7 Days)</CardTitle></CardHeader>
            <CardContent>
