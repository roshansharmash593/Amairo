...
const VendorFeedback = () => {
  ...
  const { data: feedbacks = [], isLoading } = useQuery({
    queryKey: ["vendor-all-feedback", user?.id],
    queryFn: async () => {
    ...
    <Layout>
      <section className="container py-20 md:py-28 max-w-3xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-muted-foreground">See what customers are saying about your products & delivery.</p>
        </motion.div>
        ...
        {isLoading ? (
          ...
          <Card>
            <CardContent className="pt-6 text-center py-16">
              ...
              <p className="text-sm text-muted-foreground mt-1">Feedback will appear here once customers submit reviews.</p>
            </CardContent>
          </Card>
        ) : (
                  ...
                  <CardContent className="pt-4 pb-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-xs text-muted-foreground">#{fb.order_id.slice(0, 8)}</span>
                      </div>
                      <div className="flex items-center gap-2">
