...
import { User, Phone, FileText, Car, Pencil, X, Plus, Camera, Trash2, Bike, Mail } from "lucide-react";
...
const RiderProfileCard = ({ profile, user, refetchProfile, toast }: RiderProfileCardProps) => {
  ...
  const uploadVehiclePhoto = async (file: File) => {
    ...
    const path = `${user!.id}/vehicle-photo.${ext}`;
    ...
    if (updateErr) {
      ...
      toast({ title: "🏍️ Vehicle photo updated!" });
      refetchProfile();
    }
  };
        ...
        <CardContent className="pt-5 pb-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                ...
                {profile?.vehicle_details && (
                  <span className="text-xs text-muted-foreground truncate">{profile.vehicle_details}</span>
                )}
              </div>
            </div>
          ...
          {editing ? (
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground">Vehicle Details</label>
                <Input value={vehicleDetails} onChange={(e) => setVehicleDetails(e.target.value)} placeholder="e.g. Honda Activa 6G - White" className="mt-1" maxLength={150} />
              </div>
              <div>
